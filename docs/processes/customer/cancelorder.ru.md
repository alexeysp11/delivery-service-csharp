...
- Сервис [customerbackend](../../backend/customerbackend.ru.md) проверяет, можно ли отменить заказ.
    - Проверить, изменился ли статус заказа.
    - Если статус заказа изменился, то обновить данные в БД, вывести сообщение «Заказ не может быть отменен из-за несоответствия статуса: <название статуса>» и обновить статус заказа.
