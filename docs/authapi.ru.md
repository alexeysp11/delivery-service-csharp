...
- Джобы: 
    - Пометка записей в таблице проверки кода на клиентском приложении как устаревшие и перемещение на сервис (каждые 15-30 минут): если время отправки запроса было раньше продолжительности перепроверки и не закрыта 
    ...
    - Удаление устаревших записей из проверки кода на клиентском приложении и временной регистрации на сервисе (1-7 раз в неделю)
...
4. Пользователь попадает на страницу ввода кода подтверждения.
    ...
    - Полученный ответ от сервиса добавляется в БД (неустаревшие и незакрытые записи становятся перезаписанными).
